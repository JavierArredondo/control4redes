...
int main(int argc, char **argv){
  ...
  if(strcmp(argv[1], "TCP") == 0)
  {
    ...
    congrat = bind(server_connection, (struct sockaddr *)&server, sizeof(server));
    if(congrat < 0)
    {
    ...
    while(recv(client_connection, buffer, 100, 0) != 0)
    {
      ...
      send(client_connection, "Recibido", 13, 0);
    }
  }
  else if(strcmp(argv[1], "UDP") == 0)
  {
    ...
    longc = sizeof(client); //  Tamaño de estructura del cliente
    int r;
    while((r = recvfrom(server_connection, buffer, 100, 0, (struct sockaddr *) &client, &longc)) == 100)
    {
        printf("%s\n" , buffer);
        sendto(server_connection, "Recibido", r, 0, (struct sockaddr*) &client, longc);
    }
  }
  else
  {
